...
IdrAgra now reads raw CropCoef-compatible `.tab` crop definitions from
`CropParameterPath`, and ordered rotation slots from `CropRotationFile`.
It advances phenology internally for every cell and day. Generated `CropId.dat`,
`Kcb.dat`, `CropParam.dat`, `WPadj.dat`, and `CanopyRes.dat` files are no longer read.
See [standalone phenology](standalone_phenology.md) for input and output details.
