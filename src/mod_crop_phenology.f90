...
! crop_pheno_info for details
    ...
    logical,dimension(:,:,:), pointer :: external_calendar ! true when sowing or harvest is externally specified
    logical,dimension(:,:), pointer :: suppress_irandom ! cell has an external sowing/harvest event
    ...
    integer,dimension(:,:,:), pointer :: n_external_cuts    ! number of external cuts for each cell/crop cycle
    integer,dimension(:,:,:,:),pointer:: external_cut_doy   ! sorted external cut dates
    integer,dimension(:,:,:), pointer :: ref_first_cut      ! first cut detected in the reference Kcb series
    integer,dimension(:,:,:), pointer :: ref_regrowth_start ! Reference DOY at which a multi-cut crop goes back to after an external cut (todo: currently == ref_first_cut, future version could choose ref_regrowth_start among all reference cut dates)
    integer,dimension(:,:,:), pointer :: ref_regrowth_end   ! last day of the reusable post-cut interval
   ...
   type(crop_pheno_info),dimension(:),intent(in)::info_pheno
    ...
    do j=1,size(domain%mat,2)
        do i=1,size(domain%mat,1)
            if(domain%mat(i,j)/=domain%header%nan)then
                do z=1,size(crop_mat%ii0,3)
                    slot = info_pheno(dir_phenofases(i,j))%cycle_crop_slot(soil_use(i,j),z)
                    if (slot <= 0) then
                        cycle
                    end if
...
subroutine populate_crop_pars_matrices(crop_pars_mat, info_pheno, irandom, doy, ws_idx, domain, &
                                     & soil_use, y, year_length, crop_mat)
...
subroutine populate_crop_cell(i, j, crop_pars_mat, info_pheno, irandom, doy, ws_idx, &
    ...
    type(crop_pars_matrices), intent(inout) :: crop_pars_mat
    type(crop_matrices), intent(in) :: crop_mat
    integer :: cycle_idx, active_cycle, slot, lu, doy_s
    integer :: effective_shift, cut_idx, last_external_cut
    integer :: read_crop_slot
    ...
    !%PS%: crops for which the user specifies exact sow/harvest dates do not use irandom
    effective_shift = irandom(i,j)
    if (crop_mat%suppress_irandom(i,j)) effective_shift = 0

    active_cycle = 0
    do cycle_idx = 1, size(crop_mat%ii0, 3)
        if (info_pheno(ws_idx(i, j))%cycle_crop_slot(lu, cycle_idx) <= 0) cycle
    ...
    !%PS%: for multi-cut crops, externally-prescribed cuts shift the local lookup doy to the first cropcoef-calculated cut
    if (crop_mat%n_external_cuts(i,j,active_cycle) > 0) then
        last_external_cut = 0
        do cut_idx=1, crop_mat%n_external_cuts(i,j,active_cycle)
            if (crop_mat%external_cut_doy(i,j,active_cycle,cut_idx) <= doy) then
                last_external_cut = crop_mat%external_cut_doy(i,j,active_cycle,cut_idx)
            else
                exit
            end if
        end do

        if (last_external_cut == 0) then
            ! DOY of the first prescribed cut hasn't been reached: don't allow doy_s to reach ref_first_cut (cropcoef-calculated first cut)
            doy_s = min(doy_s, crop_mat%ref_first_cut(i,j,active_cycle) - 1)
        else
            ! Perform the cut by moving doy_s to ref_first_cut (note that this means that re-growth speed becomes indipendent of weather conditions - TODO)
            doy_s = crop_mat%ref_regrowth_start(i,j,active_cycle) + doy - last_external_cut
            doy_s = min(doy_s, crop_mat%ref_regrowth_end(i,j,active_cycle))
        end if
        doy_s = max(1, min(year_length, doy_s))
    end if
